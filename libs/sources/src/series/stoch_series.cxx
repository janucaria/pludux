...
public:
   ...
   -> SeriesOutput<ValueType>
  {
    ...
    const auto nan_index = series_size - self.k_period_ + 1;
    const auto last_k_index = lookback + self.k_smooth_ - 1;
    const auto last_d_index = last_k_index + self.d_period_ - 1;
    ...
    if(last_k_index < nan_index) {
      ...
      for(std::size_t k_smooth_i = 0; k_smooth_i < self.k_smooth_;
          ++k_smooth_i) {
        const auto k_index = lookback + k_smooth_i;
        const auto begin = k_index;
        const auto end = k_index + self.k_period_;

        auto highest_high = std::numeric_limits<ValueType>::min();
        auto lowest_low = std::numeric_limits<ValueType>::max();

        for(auto i = begin; i < end; ++i) {
          const auto high = static_cast<ValueType>(self.high_[i]);
          const auto low = static_cast<ValueType>(self.low_[i]);

          highest_high = std::max(highest_high, high);
          lowest_low = std::min(lowest_low, low);
        }

        const auto stoch = 100 * (self.close_[k_index] - lowest_low) /
                           (highest_high - lowest_low);
        sum_k_stoch += stoch;
      }

      result_k = sum_k_stoch / self.k_smooth_;
    }
  ...
  {
    assert(self.high_.size() == self.low_.size());
    assert(self.high_.size() == self.close_.size());

    return self.close_.size();
  }
