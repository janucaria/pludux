...
TEST(RsiMethodTest, RunAllMethod)
{
  ...
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rsi_method, asset_snapshot[0], context),
   41.446303291958991);
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rsi_method, asset_snapshot[1], context),
   44.444444444444443);
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rsi_method, asset_snapshot[2], context),
   44.44444444444445);
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rsi_method, asset_snapshot[3], context),
   44.444444444444443);
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rsi_method, asset_snapshot[4], context), 50);
  EXPECT_TRUE(
   std::isnan(evaluate_series_method(rsi_method, asset_snapshot[5], context)));
  EXPECT_TRUE(
   std::isnan(evaluate_series_method(rsi_method, asset_snapshot[6], context)));
  EXPECT_TRUE(
   std::isnan(evaluate_series_method(rsi_method, asset_snapshot[7], context)));
  EXPECT_TRUE(
   std::isnan(evaluate_series_method(rsi_method, asset_snapshot[8], context)));
  EXPECT_TRUE(
   std::isnan(evaluate_series_method(rsi_method, asset_snapshot[9], context)));
}
