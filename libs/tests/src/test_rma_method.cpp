...
TEST(RmaMethodTest, RunAllMethod)
{
  ...
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rma_method, asset_snapshot[0], context),
   854.33056000000022);
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rma_method, asset_snapshot[1], context),
   854.16320000000019);
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rma_method, asset_snapshot[2], context),
   852.70400000000018);
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rma_method, asset_snapshot[3], context),
   850.88000000000011);
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rma_method, asset_snapshot[4], context),
   848.60000000000002);
  EXPECT_DOUBLE_EQ(
   evaluate_series_method(rma_method, asset_snapshot[5], context), 842);
  EXPECT_TRUE(
   std::isnan(evaluate_series_method(rma_method, asset_snapshot[6], context)));
  EXPECT_TRUE(
   std::isnan(evaluate_series_method(rma_method, asset_snapshot[7], context)));
  EXPECT_TRUE(
   std::isnan(evaluate_series_method(rma_method, asset_snapshot[8], context)));
  EXPECT_TRUE(
   std::isnan(evaluate_series_method(rma_method, asset_snapshot[9], context)));
}
