...
public:
  BacktestRunner(Backtest& run_backtest,
  ...
  void run(this BacktestRunner& self,
           const Store<Asset>& asset_store,
           const Store<Strategy>& strategy_store,
           const Store<Market>& market_store,
           const Store<Broker>& broker_store,
           const Store<Profile>& profile_store)
  {
    using namespace backtest;

    if(!!self.backtest_.is_failed()) {
      return;
    }

    const auto& asset_handle = self.backtest_.asset_handle();
    const auto asset_ptr = asset_store.get_if_present(asset_handle);
    if(!asset_ptr) {
      self.backtest_.is_failed(true);
      return;
    }

    const auto& strategy_handle = self.backtest_.strategy_handle();
    const auto strategy_ptr = strategy_store.get_if_present(strategy_handle);
    if(!strategy_ptr) {
      self.backtest_.is_failed(true);
      return;
    }

    const auto& profile_handle = self.backtest_.profile_handle();
    const auto profile_ptr = profile_store.get_if_present(profile_handle);
    if(!profile_ptr) {
      self.backtest_.is_failed(true);
      return;
    }

    const auto& market_handle = self.backtest_.market_handle();
    const auto market_ptr = market_store.get_if_present(market_handle);
    if(!market_ptr) {
      self.backtest_.is_failed(true);
      return;
    }

    const auto& broker_handle = self.backtest_.broker_handle();
    const auto broker_ptr = broker_store.get_if_present(broker_handle);
    if(!broker_ptr) {
      self.backtest_.is_failed(true);
      return;
    }
    ...
    auto summary = !summaries.empty()
                    ? summaries.back()
                    : BacktestSummary{self.backtest_.initial_capital()};
...
private:
   ...
   -> std::optional<TradeEntry>
  {
    ...
    if(strategy.long_entry_filter()(prev_snapshot, context)) {
      ...
      const auto risk_value =
       profile.capital_risk() * self.backtest_.initial_capital();
