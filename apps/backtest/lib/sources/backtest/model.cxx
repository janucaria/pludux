...
export module pludux.backtest:model;
...
public:
  ...
  public:
    ...
    auto retrigger(this const Pyramiding& self) noexcept -> PyramidingRetrigger
    {
      return self.retrigger_;
    }

    void retrigger(this Pyramiding& self,
                   PyramidingRetrigger retrigger) noexcept
    {
  ...
  auto operator==(const Model&) const noexcept -> bool = default;

  auto name(this const Model& self) noexcept -> const std::string&
  {
    return self.name_;
  }

  void name(this Model& self, std::string name) noexcept
  {
  ...
  auto long_position(this const Model& self) noexcept -> const Position&
  {
    return self.long_position_;
  }

  void long_position(this Model& self, Position long_position) noexcept
  {
    self.long_position_ = std::move(long_position);
  }

  auto short_position(this const Model& self) noexcept -> const Position&
  {
    return self.short_position_;
  }

  void short_position(this Model& self, Position short_position) noexcept
  {
    self.short_position_ = std::move(short_position);
  }

  auto plots(this const Model& self) noexcept
   -> const std::vector<PlotGroup>&
  {
    return self.plots_;
  }

  void plots(this Model& self, std::vector<PlotGroup> plots) noexcept
  {
    self.plots_ = std::move(plots);
  }

  auto intrabar_path(this const Model& self) noexcept -> IntrabarPath
  {
    return self.intrabar_path_;
  }

  void intrabar_path(this Model& self, IntrabarPath path) noexcept
  {
    self.intrabar_path_ = path;
  }

  auto equivalent_rules(this const Model& self,
                         const Model& other) noexcept -> bool
  {
