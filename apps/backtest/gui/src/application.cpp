...
void Application::on_update()
{
  if(!state_data_.backtests.empty()) {
    ...
    try {
      do {
        if(std::ranges::none_of(backtests, [](const auto& backtest) {
             return backtest.should_run();
           })) {
          break;
        }

        auto timestamps = backtests |
                          std::views::transform([](const auto& backtest) {
                            return backtest.get_current_asset_timestamp();
                          }) |
                          std::views::filter([](const auto& timestamp) {
                            return timestamp.has_value();
                          }) |
                          std::views::transform([](const auto& timestamp) {
                            return timestamp.value();
                          }) |
                          std::views::common;

        const auto lowest_day =
         *std::min_element(timestamps.begin(), timestamps.end());

        for(auto& backtest : backtests) {
          backtest.run(lowest_day);
        }
